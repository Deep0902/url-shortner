...
const Particles: React.FC<ParticlesProps> = ({
  ...
  // Add a ref to store the displayed (smoothed) position
  const displayedPositionRef = useRef<{ x: number; y: number }>({ x: 0, y: 0 });
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      if (moveParticlesOnHover) {
        ...
        displayedPositionRef.current.x = 0;
        displayedPositionRef.current.y = 0;
